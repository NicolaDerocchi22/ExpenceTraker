...
try {
  mongoose.connect(
    "mongodb+srv://Admin-Nicola:Nicpool09-@cluster0.itsqj.mongodb.net/expenceTrakerDB"
  );
...
app.get("/", async (req, res) => {
  ...
  speseFiltered = spese.sort((a, b) => {
    return b.data - a.data;
  });
  entrateFiltered = entrate.sort((a, b) => {
    return b.data - a.data;
  });
  ...
  res.render("Home", {
    listaSpese: speseFiltered,
    listaEntrate: entrateFiltered,
    totEntrate: tEntrate,
    totSpese: tSpese,
    totHype: Number(tHype),
    totPayPal: Number(tPaypal),
    periodo: _.capitalize(oggi),
  });
});
...
app.get("/spese", async (req, res) => {
  ...
  res.render("spese", {
    spese: spese,
    tBar: result.bar,
    tPayPal: result.payPal,
    tBenzina: result.benzina,
    tSpesa: result.spesa,
    tShopping: result.shopping,
    tRistorante: result.ristorante,
    tRegali: result.regali,
    tCasa: result.casa,
    tPrelievi: result.prelievi,
    tTelefono: result.telefono,
    tPalestra: result.palestra,
    tAltro: result.altro,
    totSpese: totSpese,
  });
});
...
app.get("/analisi", async (req, res) => {
  ...
  budgetSummary.diffShopping =
    budgetSummary.bShopping - budgetSummary.rShopping;
  budgetSummary.diffRistorante =
    budgetSummary.bRistorante - budgetSummary.rRistorante;
  ...
  budgetSummary.diffPalestra =
    budgetSummary.bPalestra - budgetSummary.rPalestra;
  budgetSummary.diffPrelievi =
    budgetSummary.bPrelievi - budgetSummary.rPrelievi;
  budgetSummary.diffTelefono =
    budgetSummary.bTelefono - budgetSummary.rTelefono;
...
app.post("/deleteSpesa", async (req, res) => {
  var id = req.body.btndeleteSpese;

  try {
    await Spesa.findByIdAndDelete(id);
  } catch (error) {
    console.log(error);
  }
...
app.post("/deleteEntrata", async (req, res) => {
  var id = req.body.btndeleteEntrate;

  try {
    await Entrata.findByIdAndDelete(id);
  } catch (error) {
    console.log(error);
  }

  res.redirect("/entrate");
});
