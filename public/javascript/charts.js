...
function loadChartBalance() {
  ...
  fetchDataChartBalance().then((res) => {
    if (res === null) {
      console.log("Bad");
    } else {
...
function loadChartCategoriesS() {
  ...
  fetchDataCategoriesChartS().then((res) => {
    if (res === null) {
      console.log("Bad");
    } else {
      optionsPieCategoryS.series = res;
      var chart = new ApexCharts(
        document.querySelector("#categoryChartS"),
        optionsPieCategoryS
      );
      chart.render();
    }
...
function loadChartCategoriesE() {
  ...
  fetchDataCategoriesChartE().then((res) => {
    if (res === null) {
      console.log("Bad");
    } else {
      optionsPieCategoryE.series = res;
      var chart = new ApexCharts(
        document.querySelector("#categoryChartE"),
        optionsPieCategoryE
      );
      chart.render();
    }
...
window.onload = () => {
  loadCharts();
  flatpickr("#exampleFormControlInput2", {
    dateFormat: "d/m/Y",
    defaultDate: new Date(),
  });
  flatpickr("#exampleFormControlInput3", {
    dateFormat: "d/m/Y",
    defaultDate: new Date(),
  });
};
